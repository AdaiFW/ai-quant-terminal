...
从最初的一句 "analyze this stock" 迭代 5 个版本，最终形成一个包含 10 条规则的系统 Prompt。它强制结构化输出、禁止模糊措辞，并将 LLM 锚定在专业分析师角色中。

**迭代历史：**

| 版本 | 问题 | 修复 |
|------|------|------|
| v1 | LLM 返回段落文本 | 新增 "Return ONLY valid JSON" 规则 |
| v2 | LLM 含糊其辞："could go either way" | 明确禁止模糊词（Rule 8） |
| v3 | LLM 使用错误的 JSON 键名 | 在 Prompt 中列举允许的键名（Rule 10） |
| v4 | 置信度分数虚高（90-95） | 新增 Rule 9：数据不足时必须降低置信度 |
| v5（生产版） | 仅靠 Prompt 不够可靠 | 三层防线：Prompt → safeJSONParse → zod 重试 |

### 3.2 生产环境 System Prompt
...
User Prompt 将结构化的行情数据直接注入 LLM 上下文：
...
- **数值精度** — 价格精确到 2 位小数，成交量带分隔符，市值用十亿单位。这些都是对 LLM 的解读辅助信号。
- **方向性信号由服务端预计算** — "Trading ABOVE 50-day MA — bullish signal" 预先完成了均线比较，而非让 LLM 自行计算。
- **空值处理** — 当均线数据不可用时，整行省略而非显示 "N/A"，减少噪声。
